...
def create_hplc_chromatogram(sample_id='DEFAULT'):
    ...
    drawing.add(String(68, 15, f'{rt1:.2f}', fontSize=7, fillColor=colors.black, textAnchor='middle'))
    drawing.add(String(98, 15, f'{rt2:.2f}', fontSize=7, fillColor=colors.black, textAnchor='middle'))
    drawing.add(String(125, 15, f'{rt3:.2f}', fontSize=7, fillColor=colors.black, textAnchor='middle'))
    drawing.add(String(158, 15, f'{rt4:.2f}', fontSize=7, fillColor=colors.black, textAnchor='middle'))
    ...
    drawing.add(String(335, 15, f'{rt8:.2f}', fontSize=7, fillColor=colors.black, textAnchor='middle'))
...
def create_locked_one_page_potency_template(
    ...
    # Calculate dates
    today = datetime.now()
    # Received: 7 business days before today
    days_back = 0
    biz_days = 0
    while biz_days < 7:
        days_back += 1
        ...
        if check_date.weekday() < 5:  # Weekday
            biz_days += 1
    ordered_date = today - timedelta(days=days_back)
    # Tested: 2 business days after received
    completed_date = ordered_date + timedelta(days=2)
    if completed_date.weekday() >= 5:  # Weekend
        completed_date += timedelta(days=2)
    expires_date = completed_date + timedelta(days=365)
    ...
    # Calculate cannabinoid values FIRST (before summary boxes)
    import random
    
    # Parse input percentages
    thc_val = float(thc_percentage)  # This is THCa for hemp
    cbd_val = float(cbd_percentage)
    
    # FOR HEMP: thc_val is actually THCa percentage
    # Delta-9 THC must be under 0.3% for hemp compliance
    thca_pct = round(thc_val, 2)  # Use the input value directly as THCa
    d9_thc_pct = round(random.uniform(0.18, 0.28), 2)  # Low delta-9 THC for hemp
    
    # Calculate Total THC using decarboxylation formula
    # Total THC = (THCa × 0.877) + Delta-9 THC
    total_thc_calculated = round((thca_pct * 0.877) + d9_thc_pct, 2)
    ...
    # Minor cannabinoids (small amounts) - use sample_id for unique values
    import hashlib
    seed = int(hashlib.md5(sample_id.encode()).hexdigest()[:8], 16)
    rng = random.Random(seed)
    cbn_pct = round(rng.uniform(0.10, 0.25), 2)
    cbg_pct = round(rng.uniform(0.15, 0.35), 2)
    ...
    # Analysis details - compact with randomized values
    analyst_num = random.randint(300, 599)
    extracted_by = random.randint(500, 799)
    weight = round(random.uniform(2.8, 3.3), 4)
    batch_num = f"DA{random.randint(100000, 999999)}"
    prep_time = f"{random.randint(1,23):02d}:{random.randint(0,59):02d}:{random.randint(0,59):02d}"
    
    analysis_info = [
        ['Cannabinoid Profile Test', f'Analyst: {analyst_num}', f'Weight: {weight}g'],
        [f'Sample Prep: {ordered_date.strftime("%Y-%m-%d")} {prep_time}', f'Extracted By: {extracted_by}', f'Batch: {batch_num}'],
